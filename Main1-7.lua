...
-- helper: sync QuickBtn visual without notification
...
TabTarget:CreateParagraph({
    Title   = "Color legend",
    Content = "Red = Hostile  |  Green = Friendly  |  Orange = Unknown\nDetection: ObjectValue Target pointing to a player = Hostile",
})
...
TabHitbox:CreateInput({
    Name = "Head Transparency", CurrentValue = "0.5", PlaceholderText = "0 – 1",
...
TabESP:CreateToggle({ Name="2D Box",        CurrentValue=false, Flag="EBox",   Callback=function(v) getgenv().ESP_Box       = v end })
TabESP:CreateToggle({ Name="Tracers",       CurrentValue=false, Flag="ETrace", Callback=function(v) getgenv().ESP_Tracer    = v end })
TabESP:CreateToggle({ Name="Highlight",     CurrentValue=false, Flag="EHL",    Callback=function(v) getgenv().ESP_Highlight = v end })
...
TabAura:CreateToggle({ Name="Team Check",   CurrentValue=false, Flag="ATC", Callback=function(v) getgenv().AuraTeamCheck  = v end })
TabAura:CreateToggle({ Name="Skip Friends", CurrentValue=false, Flag="AFC", Callback=function(v) getgenv().AuraFriendOnly = v end })
